...
initpos = [NullTetrad(np.array([0, 0, 0.8j+r(phi) * np.exp(1j * phi), -0.8j+r(phi) * np.exp(- 1j * phi)], dtype=np.complex128))
...
for m, i in zip(mu, range(len(mu))):
    ...
    plotter = PlotlyDynamicPlotter(title="",
                                   ...
                                   xrange=[-6, 6], yrange=[-6, 6], zrange=[-1, 2],
    ...
    for x0, u0, geonum in zip(initpos, initvels, range(0, len(initpos))):
        ...
        a = wave.generate_geodesic(x0, u0, (-1.3, 3), max_step=0.2, verbose=False)
        ...
        plotter.plotTrajectory3D(convertFunction(trajm), color=color, xc=plot[0], yc=plot[1], zc=plot[2], name="Geodesic {} (-)".format(geonum), t=tm, linewidth=3, dash="longdashdot")
        plotter.plotTrajectory3D(convertFunction(trajp), color=color, xc=plot[0], yc=plot[1], zc=plot[2], name="Geodesic {} (+)".format(geonum), t=tp, linewidth=3)
    ...
    plotter.export_pdf("{}_{}{}{}_mu={}.pdf".format(plotName, lab[0], lab[1], lab[2], m), eye=(.4, -1.7, 1.9), up=(0., 0., 1.))
